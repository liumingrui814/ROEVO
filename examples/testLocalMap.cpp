...
int main(int argc, char **argv) {
    ...
    visualizeMergedLocalMap(l_map, mergedCloud);
    ...
    for(int i = 0; i < clusterClouds.size(); ++i){
        ...
        *cloud_cluster = clusterClouds[i];
        ...
        viz_util::drawPointCloudColor(viewer, id, cloud_cluster, 160, 160, 160, 1);
    }
    ...
    for(int i = 0; i < l_map.mvKeyFrames.size(); ++i)
    {
        ...
        Sophus::SE3d pose_ref_cur = l_map.mvKeyFrames[i]->KF_pose_g;
