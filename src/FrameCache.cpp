...
Frame::Frame(int ID, cv::Mat imgDepth, double fx, double fy, double cx, double cy, std::vector<Edge> vEdges)
{
    //-- 设置ID
    frame_ID = ID;
    ...
    //-- 帧的原始图像的赋值
    mMatDepth = imgDepth.clone();
    mHeight = imgDepth.rows;
    mWidth = imgDepth.cols;

    //-- 使用selector提取边缘
    mvEdges = vEdges;

    assignProperty3D(); //-- 为边缘赋值深度
    edgeCullingDepth();      //-- 剔除所有深度无效的边缘以及有效边缘中的无效边缘点
    edgeCullingContinuity(); //-- 修整边缘，移除深度不均匀的间断片段

    assignPropertyIdx();//-- 为所有边缘点赋所属边缘ID信息以及边缘中的idx信息，方便检索

    constructSearchPlain();

    // isCloudGenerated = false; //-- 匹配点云被创建了才会为true
}
...
std::vector<int> Frame::edgeWiseCorrespondenceReproject(Edge& query_edge, Eigen::Matrix4d T2curr)
{
    ...
    for(int i = 0; i < num_points; ++i)
    {
        ...
        //-- 重投影得到新的投影点
        Eigen::Vector4d point = T2curr * Eigen::Vector4d(x, y, z, 1);
    ...
    for(int i = 0; i < num_points; ++i)
    {
        ...
        searchRadius(x, y, 6, neighbors_points);

        // KNNresult neighbors = searchRadius(x, y, 6);
        // std::vector<orderedEdgePoint> neighbors_points = neighbors.first;
        // std::vector<int> neighbors_edges = neighbors.second; 
...
void Frame::assignProperty3D()
{
    for(int i = 0; i < mvEdges.size(); ++i){
        ...
        //-- 在3D赋分的时候可见性分数也被计算得到了，这时则可以赋边缘的整体深度
        mvEdges[i].calcEdgeScoreViz();
    }
}
...
void Frame::assignProperty3DEach(orderedEdgePoint& pt)
{
    ...
    //-- 原本点的真实深度
    float depth_orig = mMatDepth.at<float>(y_idx, x_idx);
    ...
    for(int x_bias = -2; x_bias <= 2; ++x_bias){
        for(int y_bias = -2; y_bias <= 2; ++y_bias){
            ...
            //-- 在图像区域里的话判断深度值是否有效
            float depth = mMatDepth.at<float>(curr_y_idx, curr_x_idx);
...
std::vector<orderedEdgePoint> Frame::getCoarseSampledPoints(int bias, int maximum_point)
{
    ...
    std::sort(selectedPoints.begin(),selectedPoints.end(),
              [](orderedEdgePoint a, orderedEdgePoint b){ return a.score_depth > b.score_depth; });
    //-- 创建全黑的图像作为掩膜
    cv::Mat mask(mHeight, mWidth, CV_8UC3, cv::Scalar::all(0));
    //-- 根据掩膜进行采样排序
    std::vector<orderedEdgePoint> sampledPoints;
    for(int i = 0; i < selectedPoints.size(); ++i){
        if(mask.at<cv::Vec3b>(selectedPoints[i].y, selectedPoints[i].x) == cv::Vec3b(255, 255, 255)) continue;
        //-- 当前点可以选择
        sampledPoints.push_back(selectedPoints[i]);
        cv::circle(mask, cv::Point(selectedPoints[i].x, selectedPoints[i].y), bias, cv::Scalar(255,255,255), -1);
        if(sampledPoints.size() >= maximum_point) break;
    }
    //-- 目前是完全采样完成的所有点
    mvCoarseFeaturePoints = sampledPoints;
    return sampledPoints;
}

std::vector<orderedEdgePoint> Frame::getFineSampledPoints(int bias)
{
    ...
    for(int i = 0; i < mvEdges.size(); ++i){
        ...
        std::vector<int> index_sampled = edge.mvSampledEdgeIndex;
        //-- 获取采样的序列
    ...
    mvFineFeaturePoints = selectedPoints;

    // //-- 构造用于搜索的采样点云以及边缘点索引列表
    // mpFrameCloudFine2D.reset(new pcl::PointCloud<pcl::PointXYZ>());
    // for(int i = 0; i < selectedPoints.size(); ++i){
    //     pcl::PointXYZ pt_2d;
    //     pt_2d.x = selectedPoints[i].x;
    //     pt_2d.y = selectedPoints[i].y;
    //     pt_2d.z = 0;
    //     mpFrameCloudFine2D->push_back(pt_2d);
        
    // }
    // //-- 构造kdtree
    // kdtree.setInputCloud(mpFrameCloudFine2D);
...
void Frame::assignPropertyIdx()
{
    ...
    for(int i = 0; i < mvEdges.size(); ++i){
        ...
        for(int j = 0; j < mvEdges[i].mvPoints.size(); ++j){
            //-- 更新边缘id索引
            mvEdges[i].mvPoints[j].frame_edge_ID = edge_id;
            //-- 更新边缘点列表索引
            mvEdges[i].mvPoints[j].frame_point_index = j;
        }
    }
}
