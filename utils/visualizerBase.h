...
public:
     ...
     * @param size 点大小
     */
    void drawPoint(cv::Point3f v1, cv::Vec3b color, int size = 1);
     ...
     * @param size 线的宽度
     */
    void drawLine(cv::Point3f v1, cv::Point3f v2, cv::Vec3b color, int width = 1);
     ...
     */
    void drawCoordinate(const Eigen::Matrix4f& pose, double length=1, int width=3);
     ...
     */
    void drawTrajectory(const std::vector<Eigen::Matrix4f>& poses, bool showRotation,cv::Vec3b color, double axis_len = 0.1);
     ...
     * @param color 单一颜色
     */
    void drawPointCloud(const std::vector<cv::Point3f>& pointcloud, cv::Vec3b color, int pointSize = 1);
     ...
     */
    void drawPointCloud(const std::vector<cv::Point3f>& pointcloud, std::string axis, int pointSize = 1);
    ...
    */
    void drawCamera(Eigen::Matrix4f cameraPose, cv::Vec3b color, double camSize);
...
private:
     ...
     */
    void addPoint(cv::Point3f v1, cv::Vec3b color);

    /** 
     * @brief 封装私有函数, 封装了线渲染的glColor和glVertex+glVertex
     */
    void addLine(cv::Point3f v1, cv::Point3f v2, cv::Vec3b color);
